...
\begin{eulercomment}
Nama: Anisah Daffa Citra Nareswari\\
NIM: 22305141044\\
...
layarnya persegi atau tidak. Dan ada plot koordinat, yang dapat diatur
dengan setplot(). Pemetaan antar koordinat bergantung pada jendela
plot saat ini. Misalnya, shrinkwindow() default menyisakan ruang untuk
label sumbu dan a judul plot.

Dalam contoh, kita hanya menggambar beberapa garis acak dalam berbagai
warna. Untuk detail tentang ini fungsi, pelajari fungsi inti EMT.
\end{eulercomment}
\begin{eulerprompt}
>clg; // clear screen
...
\end{eulerprompt}
\begin{eulercomment}
Grafik perlu ditahan, karena perintah plot() akan menghapus plot\\
jendela.
...
Ini dilakukan untuk mendefinisikan jendela plot yang lebih kecil.
Perhatikan bahwa jendela ini tidak menyediakan ruang untuk sumbu label
...
angka utilitas() fungsi untuk ini.\\
\end{eulercomment}
...
area plot saat ini berubah, dan jendela diatur sehingga label memiliki
cukup ruang.
\end{eulercomment}
\begin{eulerprompt}
...
\begin{eulerttcomment}
   B_i(x) = \(\backslash\)binom\{n\}\{i\} x^i (1-x)^\{n-i\}
\end{eulerttcomment}
\begin{eulerprompt}
...
\begin{eulercomment}
1. Gambarkan kurva fungsi f(x) = x\textasciicircum{}6 + x + 3!
\end{eulercomment}
\begin{eulerprompt}
>plot2d("x^6+x+3", color=blue):
\end{eulerprompt}
\begin{eulercomment}
2. Gambarkan grafik f(x) = x\textasciicircum{}2 + 4x + 8 dan tentukan titik puncaknya!
\end{eulercomment}
\begin{eulerprompt}
>plot2d("x^2+4x+8",-5,3,1,5, color=green):
\end{eulerprompt}
\begin{eulercomment}
Berdasarkan grafik di atas, maka titik puncak dari fungsi tersebut
berada pada titik (-2, 3).

3. Gambarkan f(x) = x\textasciicircum{}2 + x - 1 dalam beberapa model grafik!
\end{eulercomment}
\begin{eulerprompt}
>figure(3,3);...
>for k=1:9; figure(k); plot2d("x^2+x-1",-2,2,grid=k); end;...
>figure(0):
\end{eulerprompt}
\begin{eulercomment}
4. Gambarkan kedua grafik berikut secara bersamaan!\\
\end{eulercomment}
\begin{eulerttcomment}
   f(x) = sin(x)
   f(x) = x^3
\end{eulerttcomment}
\begin{eulerprompt}
>plot2d("sin(x)",r=3, color=red); plot2d("x^3",color=blue,>add):
\end{eulerprompt}
\begin{eulercomment}
5. Gambarkan setengah lingkaran berwarna merah!
\end{eulercomment}
\begin{eulerprompt}
>t=linspace(0,pi,20); ...
>plot2d(cos(t),sin(t),>filled,style="/",fillcolor=red,r=1):
\end{eulerprompt}
